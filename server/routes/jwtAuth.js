...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const { username_pengguna, password, nama_depan_pengguna, nama_belakang_pengguna, email_pengguna, no_hp_pengguna } = req.body;
      ...
      const newPengguna = await pool.query(
        'INSERT INTO pengguna (username_pengguna, password, nama_depan_pengguna, nama_belakang_pengguna, email_pengguna, no_hp_pengguna, role, status) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING *',
        [username_pengguna, bcryptedPassword, nama_depan_pengguna, nama_belakang_pengguna, email_pengguna, no_hp_pengguna, 'public', 'aktif']
      );

      const initializeAlamat = await pool.query('INSERT INTO alamat (username_pengguna) VALUES ($1)', [username_pengguna]);

      const token = jwtGenerator(newPengguna.rows[0].username_pengguna, newPengguna.rows[0].role);
