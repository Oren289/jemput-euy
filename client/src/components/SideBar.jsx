...
const SideBar = () => {
  ...
  return (
    <Box
      sx={{
        "& .pro-sidebar-inner": {
          background: `${colors.blueAccent[200]} !important`,
        },
        "& .pro-icon-wrapper": {
    ...
    >
      <ProSidebar collapsed={isCollapsed}>
        <Menu iconShape='square'>
          {/* LOGO AND MENU ICON */}
          <MenuItem onClick={() => setIsCollapsed(!isCollapsed)} icon={isCollapsed ? <MenuOutlinedIcon /> : undefined} style={{ margin: "10px 0 20px 0", color: colors.grey[900] }}>
            {!isCollapsed && (
              <Box display='flex' justifyContent='space-between' alignItems='center'>
                <a className='navbar-brand fw-bold fs-5 me-md-5' style={{ color: "whitesmoke" }} href='#'>
                  Jemput Euy!
                </a>
                <IconButton>
